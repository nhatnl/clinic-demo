...
const { data: patients, status, error, refresh } = await useFetch<PatientRecord[]>('/api/patients', {
  default: () => [],
})
</script>
...
<template>
  ...
  <section class="card records-card" :aria-busy="status === 'pending'">
    <div class="card-heading">
      <div>
        <h2>Patient list <span v-if="!error && status !== 'pending'" class="count-badge">{{ patients.length }}</span></h2>
    ...
    <div v-else-if="!patients.length" class="empty-state"><h3>No patients yet</h3><p>Create a patient to get started.</p></div>
